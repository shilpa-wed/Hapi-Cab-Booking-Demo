...
import {
    findDriverPassportDetail,
    findOneDriverByCriteria
} from 'daos/driverDao';
...
module.exports = [
    ...
    },
    {
        ...
        options: {
            auth: false,
            description: 'Login api for driver using email, password!',
            notes: 'Do driver login API',
            tags: ['api', 'auth'],
            cors: true,
            validate: {
                payload: Joi.object({
        ...
        handler: async (request, h) => {
            ...
            // Check given email customer is exist or not!
            const existDriver = await findOneDriverByCriteria({
                email: email.toLowerCase()
            });
            if (!existDriver)
                throw badData(`This ${email} account does not exist!`);
            ...
            // If customer is present check the password is matching or not!
            const passportData = await findDriverPassportDetail(existDriver.id);
            if (!passportData)
                throw badData(`This ${email} account provider not exist!`);
            ...
            // Create JWT token for the user that going to be loggin
            existDriver.type = USER_TYPE.DRIVER;
            const token = await createToken(existDriver);
            ...
            // Insert record to tokens table for created jwt token!
            return h.response({ token, userId: existDriver.id });
        }
    },
    {
        method: 'GET',
        ...
        options: {
            auth: 'jwt',
            description: 'Do logout of account!',
            notes: 'do logout',
            tags: ['api', 'auth'],
            cors: true
        },
        handler: async (request, h) => {
            await db.tokens.destroy({ where: { id: request.user.tokenId } });
            return h.response({
                message: 'Logout done!'
            });
        }
